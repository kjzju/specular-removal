...
class GateGenerator(nn.Module):
	...
	def compute_attention(self, feature, patch_fb):  # [b, 192, 56, 56]
		...
		# print('cos',cos.shape)
		# print('p_matrix_h',p_matrix_h.shape)
...
if __name__ == '__main__':
	...
	opt = arg()
	model = GateGenerator(opt)
	fo, so, intensity = model(img)
	print(so.shape)
	# print(fo.shape, so.shape, intensity.shape)
	# discriminator = PatchDiscriminate(opt)
